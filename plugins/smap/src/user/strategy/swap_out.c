...
/*
 * DoSwapOut - Swap cold L2 pages to NVMe via kernel ioctl.
 *
 * Builds a MigrateMsg with bitmap indices from coldState.tracker and calls
 * ioctl(SMAP_MIG_SWAP_OUT). The kernel converts indices to PA, isolates
 * folios, and calls reclaim_pages() to force them to the swap device.
 *
 * The kernel's init_migrate_list_addr() overwrites mig_list[i].addr with
 * a kernel pointer, then __ioctl_swap_out() copies it back to user space.
 * After the ioctl, mMsg.migList[i].addr contains a kernel address that
 * free() would crash on. We keep shadow pointers to our malloc'd buffers
 * and free those instead.
 */
