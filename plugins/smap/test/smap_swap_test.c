...
int main(int argc, char *argv[])
{
    ...
    /* Explicitly enable swap for this process (enableSwap defaults to true,
     * but set it explicitly to make the intent clear for VM processes). */
    if (proc_type == 1) {
        ProcessAttr *attr = GetProcessAttr(target_pid);
        if (attr) {
            attr->enableSwap = true;
            printf("[test] enableSwap set to true for VM pid %d\n", target_pid);
        }
    }
