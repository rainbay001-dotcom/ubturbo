 ...
 *
 * Cold detection: hist_cold[nid][addr] >= threshold means the page at bitmap
 * index addr has had freq==0 for at least threshold consecutive cycles.
...
int DoSwapOut(ProcessAttr *process)
{
    ...
    for (int nid = 0; nid < SWAP_MAX_NODES; nid++) {
        ...
        for (uint64_t i = 0; i < len; i++) {
            ...
            if (addr < hist_len && hist[addr] >= threshold) {
                node_cold++;
            }
        }
        if (node_cold == 0) {
        ...
        uint64_t idx = 0;
        for (uint64_t i = 0; i < len && idx < node_cold; i++) {
            uint64_t addr = data[i].addr;
            if (addr < hist_len && hist[addr] >= threshold) {
                addrs[idx++] = addr;
            }
