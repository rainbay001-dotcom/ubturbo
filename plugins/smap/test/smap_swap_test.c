 ...
 *   max_swap_kb     - per-process cumulative swap limit in KB; 0 = forbid all swap,
 *                     omit to use no limit (smap.swap.max.kb key absent from config)
 *
 * Prerequisites:
...
/* Sentinel: max_swap_kb not specified by user — omit the key from config */
                               ...
                               uint32_t cold_threshold, uint64_t max_swap_kb)
{
    ...
    if (max_swap_kb != MAX_SWAP_KB_UNSET) {
        fprintf(f, "smap.swap.max.kb = %lu\n",         max_swap_kb);
    }
    fclose(f);
    if (max_swap_kb != MAX_SWAP_KB_UNSET) {
        printf("[test] period config written: scan=%ums  migrate=%ums  cold_threshold=%u  max_swap_kb=%lu\n",
               scan_ms, migrate_ms, cold_threshold, max_swap_kb);
    } else {
...
int main(int argc, char *argv[])
{
    if (argc < 5 || argc > 7) {
        fprintf(stderr,
                ...
                "  max_swap_kb    : per-process cumulative swap cap in KB\n"
                "                   0 = forbid all swap; omit = no limit\n",
