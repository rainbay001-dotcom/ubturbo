...
/* 0 = prohibit swap (per Q3:B); UINT64_MAX = no limit (key absent) */
...
static int32_t ConfigReadValueToUint64(char *pvalue, uint64_t *result)
{
    *result = 0;
    while (*pvalue != '\0') {
        if (*pvalue >= '0' && *pvalue <= '9') {
            uint64_t digit = *pvalue - '0';
...
static bool UpdatePeriodConfigChanged(void)
{
    ...
    if (oldScanPeriod == scanPeriod && oldMigratePeriod == migratePeriod &&
        ...
        oldFreqWt == freqWt && oldColdThreshold == coldThreshold) {
        return false;
    }
